...
class Tab2 extends Component {
  constructor(props) {
    ...
    this.provider = blockchains.map((x) =>
      setupProvider(x.rpc)
    );
  ...
  async payFromAnySource(i) {
    ...
    const result = await executePayment(raw);
  ...
  async fetchPaymentCaller(kind, data) {
    let raw;
    if (kind === 0) {
      raw = {
        nonce: data,
    ...
    const result = await fetchPayment(raw);
    console.log(result);
    return result;
  }
      ...
      <Fragment>
        <ScrollView
          ...
          contentContainerStyle={[
            ...
            { width: "90%", alignSelf: "center", minHeight: "100%" },
          ]}
        >
          {this.state.stage === 0 && (
            <Fragment>
                ...
                <CamQR
                  ...
                  callbackAddress={async (nonce) => {
                    try {
                      ...
                      const {
                        result: { address, user },
                      } = await this.fetchPaymentCaller(0, nonce);
                ...
                <CamFace
                  ...
                  onImage={async (image) => {
                    try {
                      ...
                      const {
                        result: { address },
                      } = await this.fetchPaymentCaller(1, user);
                      await this.setStateAsync({ address, user });
                      await this.getUSD();
                      await this.getBalances();
                      await this.setStateAsync({
